...
def main():
    ...
    if not cursor.fetchone():
        cursor.execute("CREATE TABLE nodes (id TEXT, label TEXT, group_id INTEGER, description TEXT)")
    ...
    cursor.execute("SELECT id, label, group_id, description FROM nodes")
    ...
    nodes = [{"id": str(n[0]), "label": n[1] if n[1] else str(n[0]), "group": n[2], "description": n[3] if n[3] else ""} for n in db_nodes]
